...
if( ADMIN_DEMO ){
	header('Location: /admin/dashboard.php?viewpage=addgame');
	exit();
}
...
$folder_name = 0;
if(isset($_POST['slug'])){
	$_POST['slug'] = strtolower(str_replace(' ', '-', $_POST['slug']));
	$folder_name = $_POST['slug'];
} else {
	$folder_name = strtolower(str_replace(' ', '-', $_POST['title']));
}
$uploadOk = 1;
...
if($uploadOk == 0){
	...
	unlink($target_file);
	header('Location: /admin/dashboard.php?viewpage=addgame&status=error&error-data='.json_encode($error));
} else {
	$zip = new ZipArchive;
	...
	//
	$_POST['redirect'] = '/admin/dashboard.php?viewpage=addgame';
	require 'request.php';
}
?>